...
(function (root, factory) {
  ...
  function buildCard(item, config) {
    ...
    var html =
      '<span class="rss-card-category" style="background:' +
      color.bg +
      ";color:" +
      color.fg +
      '">' +
    ...
    if (config.showDate && date) {
      html +=
        '<time class="rss-card-date" datetime="' +
  ...
  function attachFilters(feedData, container, grid) {
    ...
    var html =
      '<button class="rss-filter active" role="tab" aria-selected="true" data-category="all">All</button>';
    categories.forEach(function (cat) {
      html +=
        '<button class="rss-filter" role="tab" aria-selected="false" data-category="' +
        esc(cat) +
        '">' +
        esc(cat) +
        "</button>";
  ...
  function render(feedData, container, config) {
    ...
    header.innerHTML =
      '<h2><a href="' +
      esc(feedData.feedLink) +
      '" target="_blank" rel="noopener noreferrer">' +
      esc(feedData.feedTitle) +
      "</a></h2>" +
      "<p>" +
      esc(feedData.feedDescription) +
      "</p>";
